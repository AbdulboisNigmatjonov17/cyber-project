...
const DisCountProducts = ({title}) => {
    ...
    const DiscountProductsCards = productData
        .filter((item) => item.discount === true)


    return (
        <div className='Container min-h-[655px]'>
            <h3 className='font-medium text-2xl leading-8 my-20'>{title}</h3>
            <div className='Container grid grid-cols-4 gap-5'>
                {
                    DiscountProductsCards.map((item) => {
